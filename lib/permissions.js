...
export class PermissionRegistry {
  ...
  /**
   * Export permission as a share token
   * @param {string} granteeWallet - Wallet to export permission for
   * @returns {Promise<Object>} {wallet, sharedKey, grantedAt}
   */
  async exportPermission(granteeWallet) {
    if (!this.initialized) {
      await this.init();
    }
    ...
    const permission = this.permissions.get(granteeWallet);
    if (!permission) {
      throw new Error(`No permission found for ${granteeWallet}`);
    }

    // Decrypt the shared key for export
    const sharedKey = await this.getSharedKey(granteeWallet);
  ...
  /**
   * Import a permission from external source
   ...
   */
  async importPermission(sourceWallet, sharedKey) {
    if (!this.initialized) {
      await this.init();
    }

    // Store the permission (encrypt the shared key)
    const encryptedKey = await createEncryptedPayload(
      sharedKey,
      this.config.walletPubkey,
      this.config.walletSecretKeyBase58,
    ...
    const permission = {
      grantee: this.config.walletPubkey, // We are the grantee
      sharedKey: encryptedKey,
      grantedAt: Date.now(),
      grantedBy: sourceWallet,
      imported: true
    };

    this.permissions.set(sourceWallet, permission);
    this._persist();

    this.logger.info(`Imported permission from ${sourceWallet}`);
